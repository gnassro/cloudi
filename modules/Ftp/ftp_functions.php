...
function check_ftp_connexion(&$ftp, $ftp_server, $ftp_username, $ftp_userpass, $current_dir = "/", $port = 21, $type = null)
{

    $ftp = new Ftp;
    // Opens an FTP connection to the specified host*

    $ftp->connect(str_replace('ftp://', '', $ftp_server), $port);
    ...
    return is_array($ftp->nList("/"));


}

function show_ftp_content($ftp, $clitem, $folder)
{
    /**
     * @var $ftp Ftp
     */
    ...
    foreach ($file_list as $value) {
        $name = preg_split("/\s+/", $value, 9)[8];
        $size = (preg_split("/\s+/", $value, 9)[0][0] == 'd') ? 'Folder' : preg_split("/\s+/", $value, 9)[4];
        $img = ($size == 'Folder') ? '<img src="style/img/folder.png">' : '<img src="style/img/file.png">';
        $last_time = preg_split("/\s+/", $value, 9)[5] . " " . preg_split("/\s+/", $value, 9)[6] . " " . preg_split("/\s+/", $value, 9)[7];
        $perm = substr(preg_split("/\s+/", $value, 9)[0], 1);
        $grp_own = preg_split("/\s+/", $value, 9)[2] . ":" . preg_split("/\s+/", $value, 9)[3];
...
function check_ftps_connexion(&$ftp, $ftp_server, $ftp_username, $ftp_userpass, $current_dir = "/", $port = 21, $type = null)
{

    $ftp = new Ftp;
    // Opens an FTP connection to the specified host*

    $ftp->sslConnect(str_replace('ftp://', '', $ftp_server), $port);
